...
def video_to_openai(video: LMVideoPart) -> dict[str, Any]:
    # Emit a `video` content block (symmetric with `_media_dict_to_video_part`) rather than
    # collapsing into a `file` block, so `media_type` survives the round-trip. Without it, providers
    # such as Gemini/Vertex must HTTP-fetch the source to infer the MIME type (#9899).
...
def binary_to_openai(binary: LMBinaryPart) -> dict[str, Any]:
    ...
    # Re-emit provider-specific keys (format, detail, video_metadata, ...) preserved on the way in;
    # typed fields above win over stashed metadata (#9898).
    for key, value in binary.metadata.items():
