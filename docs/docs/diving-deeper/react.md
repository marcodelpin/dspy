...
Malformed LM responses are handled the same way. When the reasoning step cannot be parsed (an `AdapterParseError`, for example the model emits a `tool_args` header instead of `next_tool_args`), ReAct records the step with the fields that did parse, fills the rest with placeholders, and adds an `observation_i` describing the expected versus found fields, so the step still contributes its four trajectory keys. A parse failure in the final extraction step is retried once with that feedback appended to the prompt before the error is surfaced.
