...
def _binary_dict_to_part(file: dict[str, Any]) -> LMBinaryPart:
    ...
    #
    # LAYERED, not replaced: upstream's legacy_content_block fires only on those two
    # lossless-failure cases, while the #9898 passthrough carries unconsumed provider keys
    # on EVERY branch. A plain data: URI block carrying "detail" keeps it here and would
    # lose it under upstream's version alone, so neither is a superset of the other.
    ...
    if file_data is not None:
        ...
        if legacy_block is not None:
            metadata["legacy_content_block"] = legacy_block
