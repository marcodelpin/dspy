...
def _encode_image_from_url(image_url: str, verify: bool = True, timeout: float | None = 30.0) -> str:
    ...
    Args:
        ...
        verify: Whether to verify SSL certificates. Set to False for self-signed certs.
        timeout: Per-read (inactivity) timeout in seconds. ``None`` falls back to the helper's
            default per-read timeout; unlike upstream it cannot hang indefinitely, because the
            total wall-clock deadline enforced by ``download_bytes`` still applies.
    """
    response = download_bytes(image_url, verify=verify, timeout=timeout)
