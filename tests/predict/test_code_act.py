...
def test_codeact_code_parse_failure():
    ...
    res = program(question="What is 1+1?")
    assert res.answer == "2"
    # Fork behavior (#9236): _parse_code validates syntax up front via ast.parse, so the
    # malformed snippet is rejected as a parse failure (precise line/message) and never
    # reaches the interpreter -- no generated_code_0 key is recorded for that iteration.
    assert res.trajectory == {
        "observation_0": "Failed to parse the generated code: Error: Syntax error at line 1: "
        "'(' was never closed",
